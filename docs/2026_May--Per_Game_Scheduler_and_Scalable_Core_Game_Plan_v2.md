...
- and the remaining product-policy gap is now mostly about completion timing and cutoff policy rather than missing machinery: legacy rows still need to drain through the automatic mode/access backfill before the old missing-mode fallback can be safely tightened.
