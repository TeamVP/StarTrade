...
- `src/features/admin/components/DatabaseScreen.tsx` plus `convex/admin/queries.ts:getDatabaseHealth` now surface sampled missing metadata counts and a resumable “Backfill Metadata” action so operators can converge legacy rows without manual table edits or relying on newest-row sampling,
...
1. Continue draining the remaining legacy `sim_games.mode` backlog until missing-mode rows are effectively gone, then remove any now-unused fallback reporting or compatibility branches.
2. Continue write-side conquest-mode suppression for other derived history families that are already disabled on the read side.
3. Carry the new retention-policy seam into any remaining transcript-heavy trader tables instead of adding more one-off pruning constants.
4. Continue moving finished-game read dependencies off live runtime tables and off unnecessary result-row joins or scans when durable snapshots and indexes already exist, then remove fallback paths once old rows have converged.
5. Continue replacing legacy `StarTrade` naming in user-facing docs and surfaces with `StarStrat`, while keeping repo/path identifiers stable until a dedicated slug migration is planned.
...
- metadata convergence is now also driven by a persisted cron-backed sweep state, so legacy access/mode/community fields continue draining automatically even when no admin is sitting on the database screen,
- that automatic metadata sweep now runs a small bounded multi-pass batch per cron invocation instead of advancing only a single page every interval, which materially shortens time-to-convergence without turning the job into an unbounded maintenance loop,
...
- legacy `sim_games.mode` resolution is now centralized on a conquest-first fallback: unresolved rows still prefer mission metadata, but only retain `trader_economy` when active trader runtime rows prove that mode is still live, and the automatic metadata sweep now persists that shared resolution instead of patching raw `trader_economy` by default,
...
- the remaining background-trader write helpers now also enforce `trader_economy` as a runtime invariant at the helper boundary, so stale jobs or future callers fail fast instead of treating conquest-mode trader suppression as a convention,
- that helper-boundary invariant now also has a direct regression test seam that calls the real trader write helpers with a minimal mutation context and proves conquest-mode games are rejected before any trader-side query or write runs,
- `eco_system_outputs` is now effectively a draining legacy/admin-only table: the turn runtime no longer writes fresh per-system output transcript rows, and post-commit maintenance now deletes any remaining rows in bounded batches instead of preserving a recent window for a table with no live read path,
- `eco_market_snapshots` now retain only the current turn for normal trader-economy games, because the live economy/admin screens only read current-turn prices and the older commodity-history query currently has no caller,
- non-trader games now also drain any leftover legacy `eco_market_snapshots` rows in bounded post-commit batches, so conquest-first games stop carrying stale market-history transcripts that no live reader can reach,
- trader-economy games now also drop cancelled `eco_bg_traders` rows on the next post-commit maintenance pass instead of retaining them in the same rolling history window as delivered voyages, because no live trader screen reads cancelled-voyage history,
- the dead `listRecentDeliveries` API plus the unused cancelled/all-status branches in `listTradersWithDetails` were removed, so the trader admin surface now matches the actual live tabs instead of preserving extra transcript query paths with no caller,
- the dead historical commodity-history API has now been removed from `convex/eco/queries.ts`, and the orphaned `eco_market_snapshots.by_gameId_and_commodity` index has been dropped from schema so the backend surface matches the current-turn-only market snapshot policy,
- finished-game cleanup now prioritizes transcript/detail tables (`sim_events`, staged-turn rows, turn rows, and economy transcripts) ahead of heavier core-state deletes, so official cleanup collapses operational history earlier in the wipe lifecycle instead of leaving the highest-volume detail for the tail end of the batch chain,
- `/admin/db` no longer reports `eco_system_outputs` as an active or legacy-trader pressure metric, because that table is now fully in the legacy-drain path rather than a meaningful live diagnostic surface,
- staged-turn bookkeeping no longer treats `eco_system_outputs` as an active simulation transcript table, so preparation snapshots and staged insert ordering now match the runtime’s post-removal write surface while legacy wipe cleanup still handles old rows,
...
- and the remaining product-policy gap is now mostly about completion timing and cutoff policy rather than missing machinery: the shared missing-mode fallback is now tightened to `conquest_core`, the temporary fallback/backfill observability counters have been removed from the admin sweep surface, and the remaining work is finishing the explicit metadata drain so the last compatibility branches can eventually be deleted.
...
- May 18, 2026: Moved metadata convergence off the admin page into a persisted `admin_metadata_backfill_state` plus scheduled internal sweep, surfaced last-run and last-completed-sweep health on the database admin screen, and made each automatic sweep run several bounded passes so legacy metadata drains materially faster.
- May 19, 2026: Stopped writing fresh `eco_system_outputs` rows from turn economy processing after confirming no live runtime or player query consumes them, and changed post-commit maintenance to drain any remaining rows in bounded batches instead of keeping a rolling history window.
- May 19, 2026: Tightened `eco_market_snapshots` retention to the current turn for normal trader-economy games after confirming the only historical snapshot query has no live caller while current-turn prices are still used on the economy/admin screens.
- May 19, 2026: Tightened legacy missing-mode resolution to default unresolved `sim_games.mode` rows to `conquest_core`, with a shared exception for rows that still show active trader runtime state, and moved the metadata sweep onto that same resolver so the backlog drains under the new policy instead of backfilling raw `trader_economy`.
- May 19, 2026: Added bounded non-trader cleanup for leftover `eco_market_snapshots` rows and moved transcript/detail tables to the front of the finished-game wipe order so conquest games and postgame cleanup shed unreachable operational history sooner.
- May 19, 2026: Removed the dead `listRecentDeliveries` API and the unused cancelled/all-status trader query branches, and changed trader post-commit maintenance to drain cancelled `eco_bg_traders` rows immediately because no live screen consumes cancelled-voyage history.
- May 19, 2026: Removed the unused `getCommodityHistory` query from `convex/eco/queries.ts` and deleted the now-unused `eco_market_snapshots.by_gameId_and_commodity` index from schema, keeping the backend contract aligned with the current-turn-only market snapshot policy.
- May 19, 2026: Removed `eco_system_outputs` from backend and frontend database-health diagnostics now that the table is no longer written and is only being drained through maintenance/cleanup.
- May 19, 2026: Removed `eco_system_outputs` from staged-turn table metadata and staged insert ordering, keeping preparation bookkeeping aligned with the runtime now that this transcript table is no longer produced.
- May 19, 2026: Removed the temporary mission-backed versus fallback mode counters from the metadata backfill result, sweep state, and admin database screen now that missing-mode resolution has converged on the shared conquest-first policy.
- May 19, 2026: Added shared `trader_economy` invariant enforcement for the background-trader write helpers (`applyBackgroundTrade`, setup/spawn/deliver helpers, and NPC trader-limit automation), so conquest-mode games now reject those write paths even if a stale scheduler job or future caller reaches them.
- May 19, 2026: Added a focused regression test seam for those trader helper invariants, calling the real write helpers with a minimal fake mutation context to prove conquest-mode games fail before any trader-history query or trader-row write can run.
...
- May 18, 2026: Added review backlog summary cards plus canned moderation-note presets to the admin mission and strategy catalogs, keeping the queue pressure and common moderation reasons visible directly on the working screens without reopening `/admin/moderation` for context.
