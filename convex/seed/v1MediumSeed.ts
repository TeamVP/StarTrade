...
export async function seedV1MediumMap(
  ctx: MutationCtx,
  gameId: Id<"sim_games">,
  mapKey: string,
  gameSeed: string,
  ...
  if (systemAurora === undefined || systemIron === undefined) {
    throw new Error("v1-medium seed: homeworld IDs missing.");
  }
  ...
  for (const lane of V1_MEDIUM_LANE_KEYS) {
    ...
    if (
      ...
      throw new Error(`v1-medium seed: unknown lane ${lane.fromKey}-${lane.toKey}`);
    }
    const { distance, travelCost } = makeMediumLinkMetrics(a.x, a.y, b.x, b.y);
  ...
  return {
    systems: V1_MEDIUM_SYSTEMS.length,
